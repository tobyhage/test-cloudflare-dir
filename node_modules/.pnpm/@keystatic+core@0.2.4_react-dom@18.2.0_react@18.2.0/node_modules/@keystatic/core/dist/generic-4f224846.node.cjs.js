...
const listCollection = cache(async function listCollection(collectionPath, glob, formatInfo, extension, fsReader) {
  ...
  return (await Promise.all(entries.map(async x => {
    if (formatInfo.dataLocation === 'index') {
      ...
      if (!(await fsReader.fileExists(cloudImagePreview.getEntryDataFilepath(`${collectionPath}/${x.name}`, formatInfo)))) {
        return [];
      }
      return [x.name];
    } else {
...
function collectionReader(collection, config, fsReader) {
  ...
  const collectionPath = cloudImagePreview.getCollectionPath(config, collection);
  const collectionConfig = config.collections[collection];
  const schema = cloudImagePreview.object(collectionConfig.schema);
  const glob = cloudImagePreview.getSlugGlobForCollection(config, collection);
  const extension = cloudImagePreview.getDataFileExtension(formatInfo);
  const read = (slug, ...args) => {
    var _args$;
    return readItem(schema, formatInfo, cloudImagePreview.getCollectionItemPath(config, collection, slug), (_args$ = args[0]) === null || _args$ === void 0 ? void 0 : _args$.resolveLinkedFiles, `"${slug}" in collection "${collection}"`, fsReader, slug, collectionConfig.slugField, glob);
  };
...
const readItem = cache(async function readItem(rootSchema, formatInfo, itemDir, resolveLinkedFiles, debugReference, fsReader, ...slugInfo) {
  const dataFile = await fsReader.readFile(cloudImagePreview.getEntryDataFilepath(itemDir, formatInfo));
  ...
  let validated;
  try {
    validated = cloudImagePreview.parseProps(rootSchema, loaded, [], [], (schema, value, path, pathWithArrayFieldSlugs) => {
    ...
    if (contentFieldPathsToEagerlyResolve !== null && contentFieldPathsToEagerlyResolve !== void 0 && contentFieldPathsToEagerlyResolve.length) {
      await Promise.all(contentFieldPathsToEagerlyResolve.map(async path => {
        const parentValue = cloudImagePreview.getValueAtPropPath(validated, path.slice(0, -1));
    ...
    const formatted = cloudImagePreview.formatFormDataError(err);
...
function singletonReader(singleton, config, fsReader) {
  ...
  const schema = cloudImagePreview.object(config.singletons[singleton].schema);
  const read = (...args) => {
    var _args$2;
